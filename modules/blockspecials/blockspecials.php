...
class BlockSpecials extends Module
{
	...
	{
		return (parent::install() AND $this->registerHook('rightColumn')  AND $this->registerHook('header') AND $this->registerHook('productPromotions'));
	}
	...
	public function hookProductPromotions($params)
	{
		if (Configuration::get('PS_CATALOG_MODE'))
			return ;
		...
		$productos = Product::getPricesDrop($this->context->language->id);
		if ($productos[0][id_product] == null)
			$productos = array();
		$this->smarty->assign(array(
			'products' => $productos,
			'special' => $special,
